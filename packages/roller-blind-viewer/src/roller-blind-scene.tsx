...
import { sanitizeRollerBlindSize } from "./roller-blind-dimensions";

import type { RollerBlindSceneProps } from "./types";
...
type ViewerSceneContentProps = Required<
  Pick<RollerBlindSceneProps, "width" | "height" | "depth" | "fabricColor" | "hardwareColor" | "wallColor">
>;
...
/**
 * Интерактивная 3D-сцена рулонной шторы (React Three Fiber): комната, окно, полотно и орбитальное вращение.
 *
 * @param props.width - Ширина шторы в миллиметрах.
 * @param props.height - Высота в миллиметрах.
 * @param props.depth - Глубина системы в миллиметрах; по умолчанию 140.
 * @param props.fabricColor - Цвет ткани (CSS color).
 * @param props.hardwareColor - Цвет фурнитуры.
 * @param props.wallColor - Цвет стен; по умолчанию `#dce9ff`.
 * @param props.className - Дополнительные классы для контейнера canvas.
 * @returns Контейнер с `role="img"` и вложенным `<Canvas />`.
 */
export const RollerBlindScene = ({
  width,
  height,
  depth = 140,
  fabricColor,
  ...
  wallColor = "#dce9ff",
  className,
}: RollerBlindSceneProps) => {
  const { width: safeWidth, height: safeHeight, depth: safeDepth } = sanitizeRollerBlindSize(
    width,
    height,
    depth,
  );
  const rootClassName = className ? `roller-blind-scene ${className}` : "roller-blind-scene";
